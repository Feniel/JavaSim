    ...
    public void run ()
    {
        ...
        while (!terminated())
        {
            try
            {
                ...
                {
                    if (Coordinator.SignalQ.isEmpty())
                        System.out
                                .println("Error - signal caught, but no message given!");
                    else
                    {
                        j = Coordinator.SignalQ.dequeue();
                        Coordinator.SignalledJobs++;
                    }
                }

                if (Coordinator.SignalledJobs == 2)
                    terminate();
            }
...
The actual quantile value is provided by the getValue() method.
